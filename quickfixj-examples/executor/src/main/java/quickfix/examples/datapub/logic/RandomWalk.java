/**
 * @author Adri�n Rodr�guez Bazaga 
 * @version 1.0.0
 * @date 25 April 2016
...
package quickfix.examples.datapub.logic;
...
public class RandomWalk {
  ...
  private DirectionEnum calculateNextStep() {
    final int NUMBER_OF_DIRECTIONS = 2;
  ...
  /**
   * Calculate a random 幅度 (from the available ones)
   * @return
   */
  private int calculateNextFd() {
    final int NUMBER_OF_DIRECTIONS = 10;
    int fd = getRandomNumberGenerator().nextInt(NUMBER_OF_DIRECTIONS);
    return fd;
  }
  ...
  public boolean calculateNextPosition() {
    ...
    if (!boundsReached(getCurrentPoint())) {
      ...
      switch(chosenDirection) {
        case UP: newPosition = new Point((int)getCurrentPoint().getX()+1, (int)(getCurrentPoint().getY() - calculateNextFd()));
                               break;
        case DOWN: newPosition = new Point((int)getCurrentPoint().getX()+1, (int)(getCurrentPoint().getY() + calculateNextFd()));
                                 break;
